...
public class TwoWheelLocalizer extends Localizer { // todo: make two wheel odo work
    ...
    private double previousNavxOrientation;
    private double deltaRadians;
    private double totalHeading;
    public static double FORWARD_TICKS_TO_INCHES = 0.00072164535;
    public static double STRAFE_TICKS_TO_INCHES =0.00072364535; //8192*1*2* Math.PI*0.94488189;//0.0005; //;

    public static double parallelX=0;
    public static double parallelY=-4.7244095;
    public static double perX=0;
    public static double perY=0;
    ...
    public TwoWheelLocalizer(HardwareMap map, Pose setStartPose) {
        ...
        forwardEncoderPose = new Pose(parallelX, parallelY, 0);
        strafeEncoderPose = new Pose(perX, perY, Math.toRadians(90));
...
//        previousIMUOrientation = MathFunctions.normalizeAngle(imu.getRobotYawPitchRollAngles().getYaw(AngleUnit.RADIANS));
        previousNavxOrientation = MathFunctions.normalizeAngle(gyro.getAngularOrientation(AxesReference.EXTRINSIC, AxesOrder.ZYX, AngleUnit.RADIANS).firstAngle);
        deltaRadians = 0;
    }
    ...
    public void updateEncoders() {
        ...
        double currentIMUOrientation = MathFunctions.normalizeAngle(gyro.getAngularOrientation(AxesReference.EXTRINSIC, AxesOrder.ZYX, AngleUnit.RADIANS).firstAngle);
        deltaRadians = MathFunctions.getTurnDirection(previousNavxOrientation, currentIMUOrientation) * MathFunctions.getSmallestAngleDifference(currentIMUOrientation, previousNavxOrientation);
        previousNavxOrientation = currentIMUOrientation;
...
//        previousIMUOrientation = currentIMUOrientation;
    ...
    @Override
    public IntegratingGyroscope getNavx() {
        return gyro;
    }
}
